...
let tiltX=0,tiltY=0,expectTiltX=0,expectTiltY=0;
function tilt(e){ 
  let x = dimensions.width * 8 * (e.clientX / dimensions.width - 0.5)
  let y = dimensions.height * 8 * (e.clientY / dimensions.height - 0.5)
  tiltX += (x - tiltX)/16
  tiltY += (y - tiltY)/16
}
let cWidth,cHeight,starCounter;
starCounter = Math.floor(Math.sqrt(window.innerWidth*window.innerWidth+window.innerHeight*window.innerHeight)/10)
...
let star = {
  ...
  getCoordinates:function(object,type){
    let vect = {
      ...
      y:Math.floor(Math.random() * canvas.height * 0.8 + canvas.height * 0.1),    
...
function mouseMoveHandler(e){
  ...
  if(e.target.id = "canvas-main"){
    ...
    attributes = cursor.getBoundingClientRect()
    cursorAttributes.width = attributes.width
    cursorAttributes.radius = cursorAttributes.width/2 - 1
    cursorAttributes.x = attributes.x + attributes.width/2 
    cursorAttributes.y = attributes.y + attributes.width/2
  }
  disableInput = false
...
function newBornStar(star){
  ...
  star.x = Math.floor(Math.random() * canvas.width * 0.8 + canvas.width * 0.1)
  star.y = Math.floor(Math.random() * canvas.height * 0.8 + canvas.height * 0.1)
  star.elapsedLife = 0
...
function update(timestamp){
  ...
  fpsEl.innerHTML = `FPS: ${Math.floor(1000000/deltaTime)/1000}`
  // -- graphics paint -- //
  ...
  for(let i=0;i<starArray.length;i++){
    ...
    if(star.isSwirling == true){
      if(star.swirlIterate > 0){
        ...
        star.swirlIterate -= 0.5+(Math.random()*1)*deltaTime+(star.swirlIterate*8/star.swirlCount)*star.swirlMagnitude/2
      }else{
        star.isSwirling = false;
        star.swirlCount = 0;
      }
    ...
    if(star.isSwirling == false){
      if(Math.abs(star.velocityX/star.velocityXMemo) > 1.1 || Math.abs(star.velocityY/star.velocityYMemo) > 1.1){
        if(Math.abs(star.velocityX/star.velocityXMemo) > 1.1){
          star.velocityX *= (1 - 0.01*(star.swirlMagnitude+0.2)/2*(deltaTime/6))
        }
    ...
    // end of swirl
    if(star.x > canvas.width * 1.2|| star.y > canvas.height * 1.2|| star.x < 0 - canvas.width * 0.2 || star.y < 0 - canvas.width * 0.2){
      newBornStar(star)
    }
